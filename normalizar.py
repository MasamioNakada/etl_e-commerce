...
capital_faderal = ['Capital', 'Capital Federal', 'CapFed', 'Cap. Fed.', 'Cap.   Federal']
...
def normalizacion_sucursales(df):
    ...
    filter_provincia = lambda x: process.extractOne(x, choices_provincia)[0] if x in df.Provincia.unique() else x
    filter_localidad = lambda x: process.extractOne(x, choices_localidad)[0] if x in df.Localidad.unique() else x
